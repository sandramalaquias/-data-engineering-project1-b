...
query1_create = query1_create + """(sessionId int, itemInSession int, artist text, song text, length float, 
...
query3_create = query3_create + """(song text, userId int, firstName text, lastName text, 
...
query1_insert = "INSERT INTO session_library (sessionId, itemInSession, artist, song, length)"
query2_insert = "INSERT INTO user_library (userId, sessionId, itemInSession, artist, song, firstName, lastName)"
query3_insert = "Insert INTO song_library (song, userId, firstName, lastName)"
...
query1_select = "select artist, song from session_library WHERE sessionId = %s and itemInSession = %s"

## Query 2: Give me only the following: name of artist, song (sorted by itemInSession) and user (first and last name)\
## for userid = 10, sessionid = 182

query2_select = "select artist, song, firstName, lastName from user_library WHERE userId = %s and sessionId = %s"

## TO-DO: Query 3: Give me every user name (first and last) in my music app history who listened to the song '
## 'All Hands Against His Own'

query3_select = "select firstName, lastName from song_library WHERE song = %s"
